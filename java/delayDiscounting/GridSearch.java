...
public class GridSearch
{
	...
	private static boolean dataIsSeparable(
		...
		final double p_k)
	{
		for(final ResponseData response: p_responses)
		{
			if(response.isAttentionCheck()
				|| response.getResponseType() == ResponseType.NONE)
			{
				continue;
			}
		...
		final double p_maxKValue)
	{
		...
		for(double k = p_minKValue; k < kLimit; k += K_STEP)
		{
			for(double sensitivity = Constants.MIN_SENSITIVITY; sensitivity <= Constants.MAX_SENSITIVITY; sensitivity += SENSITIVITY_STEP)
			{
				final double a = Functions.getA(sensitivity);
				final double lnLikelihood = Functions.lnProbOfResponses(a, k,
					p_responses);
