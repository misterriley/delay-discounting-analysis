...
public class DataLoading
{
	...
	public static SubjectData[] loadSubjectAndResponseData()
	{
		...
		for (final File toRead : chooser.getSelectedFiles())
		{
			...
			int linesRead = readOneSubjectFile(subjectsMap, stimulusMap, toRead, "ASCII", false);
			if (linesRead == 0)
			{
				System.out.println("reading in Unicode");
				linesRead = readOneSubjectFile(subjectsMap, stimulusMap, toRead, "Unicode", true);
			}
	...
	private static ResponseType convertStringToResponse(final String p_stimulusResponseString)
	{
		if (p_stimulusResponseString.equals("") || p_stimulusResponseString.equalsIgnoreCase("null"))
		{
		...
		throw new RuntimeException("Invalid response string: " + p_stimulusResponseString);
	}

	private static boolean decideIfLieTrial(final int kIndex, final String[] bits)
	{
		final boolean isAttentionCheck =
			kIndex >= bits.length ? false : bits[kIndex].equals(RESP_ATTENTION_CHECK_KEYWORD);
		return isAttentionCheck;
	}

	private static int findIndex(final String[] p_array, final String p_searchItem)
	{
		final String searchItemInQuotes = "\"" + p_searchItem + "\"";

		for (int i = 0; i < p_array.length; i++)
		{
			if (p_array[i].equals(p_searchItem) || p_array[i].equals(searchItemInQuotes))
			{
				return i;
			}
		}
	...
	{
		final Map<String, StimulusData> ret = new HashMap<>();
		BufferedReader reader = null;
		try
		{
			final URL toDisplay = MainController.class.getResource(STIM_DATA_FILE);

			reader = new BufferedReader(new InputStreamReader(toDisplay.openStream()));
			...
			while (true)
			{
				...
				final StimulusData sd = new StimulusData(id, choice1, choice1Delay, choice2, choice2Delay, k);
				ret.put(id, sd);
			}
		}
	...
	{
		final Map<Integer, SubjectData> ret = new HashMap<>();
		...
		try
		{
			...
			reader = new BufferedReader(new InputStreamReader(dataFileURL.openStream()));

			final String header = reader.readLine();
			final String[] headerBits = header.split(SUBJ_DELIMITER);

			final int subjectIDIndex = findIndex(headerBits, SUBJ_SUBJ_NUMBER_TAG);
			...
			while (true)
			{
				...
				final SubjectData subject = new SubjectData(subjectID, sex, adhdTdGroup);
	 ...
	 * @return
	 */
	private static boolean messySanityCheck(final double p_choice1, final double p_choice2)
	{
		...
		{
			throw new RuntimeException("Sanity check failed: " + p_choice1 + " " + p_choice2);
		}
	...
	private static int parseNumDays(final String p_string)
	{
		...
		if (weekIndex == -1)
		{
			throw new RuntimeException(p_string + ": expecting format 'x weeks'");
		}
	...
	private static int parseTypeAFile(
		...
		final int stimulusResponseIndex,
		final String p_sourceFile)
	{
		final int subjectIDIndex = findIndex(headerBits, RESP_SUBJECT_ID_HEADER_TAG);
		final int choice1DelayIndex = findIndex(headerBits, RESP_CHOICE_1_DELAY_TAG);
		final int choice1Index = findIndex(headerBits, RESP_CHOICE_1_TAG);
		final int choice2DelayIndex = findIndex(headerBits, RESP_CHOICE_2_DELAY_TAG);
		...
		final int trialTypeIndex = findIndex(headerBits, RESP_TRIAL_TYPE_TAG);
		final int orderIndex = findIndex(headerBits, RESP_EXPERIMENT_ORDER_TAG);
		...
		try
		{
			...
			while (true)
			{
				...
				final boolean messySanityCheck = messySanityCheck(choice1, choice2);
				...
				final boolean isValidTrial = !isAttentionCheck
					...
					&& reactionTime >= RESP_RT_MINIMUM_MS
					&& !isThrowawayTrial;

				if (!isThrowawayTrial && isValidTrial && isMessyTrial != messySanityCheck)
				{
					throw new RuntimeException("Sanity check failed: " + choice1 + " " + choice2);
				}
	...
	private static int parseTypeBFile(
		...
		throws IOException
	{
		final int stimulusResponseIndex = findIndex(headerBits, B_STIMULUS_RESPONSE_HEADER_TAG);
		final int subjectIDIndex = findIndex(headerBits, RESP_SUBJECT_ID_HEADER_TAG);
		final int timeIndex = findIndex(headerBits, RESP_TIME_TAG);
		final int dateIndex = findIndex(headerBits, RESP_DATE_TAG);
		final int trialTypeIndex = findIndex(headerBits, B_TRIAL_TYPE_TAG);
		final int orderIndex = findIndex(headerBits, RESP_EXPERIMENT_ORDER_TAG);
		final int stimulusIndex = findIndex(headerBits, B_STIMULUS_ID_HEADER_TAG);
		...
		while (true)
		{
			...
			final boolean isValidTrial = response != ResponseType.NONE
				&& reactionTime >= RESP_RT_MINIMUM_MS
				&& !throwawayIndicator.equals(RESP_THROWAWAY_KEYWORD);
	...
	private static int readOneSubjectFile(
		final Map<Integer, SubjectData> p_subjectsMap,
		final Map<String, StimulusData> p_stimulusMap,
		final File p_sourceFile,
		final String p_encoding,
		final boolean p_showException)
	{
		BufferedReader reader = null;
		try
		{
			reader = new BufferedReader(new InputStreamReader(new FileInputStream(p_sourceFile), p_encoding));
			skipLines(reader, RESP_LINES_TO_SKIP);
			final String header = reader.readLine();
			final String[] headerBits = header.split(RESP_DELIMITER);

			final int stimulusResponseIndex = findIndex(headerBits, RESP_STIMULUS_RESPONSE_HEADER_TAG);
			if (stimulusResponseIndex != -1)
			{
				return parseTypeAFile(p_subjectsMap, reader, headerBits, stimulusResponseIndex, p_sourceFile.getName());
			}

			return parseTypeBFile(p_subjectsMap, p_stimulusMap, reader, headerBits, p_sourceFile.getName());
		}
		catch (final Exception ex)
		{
			...
			{
				ex.printStackTrace();
				JOptionPane.showMessageDialog(null, "Error: unable to read " + p_sourceFile.getName());
			}
		}
		finally
		{
	...
	private static void skipLines(final BufferedReader p_reader, final int p_linesToSkip)
	{
