...
public class Functions
{
	...
	public static double calculateBreakEvenKValue(final double p_choice1,
		final int p_choice1Delay, final double p_choice2,
		final int p_choice2Delay)
	{
		final double numerator = p_choice2 - p_choice1;
		final double denominator = p_choice1
			* p_choice2Delay - p_choice2
			* p_choice1Delay;
	...
	{
		return calculateBreakEvenKValue(p_data.getChoice1(),
			p_data.getChoice1Delay(), p_data.getChoice2(),
	...
	public static double[] getMaximumLikelihoodConstants(
		final Collection<ResponseData> p_responses, final double p_minKValue,
		final double p_maxKValue)
	{
		...
		return GridSearch.getMaximumLikelihoodConstants(p_responses,
			p_minKValue, p_maxKValue);
	}

	private static double hyperbolicPresentValue(final double p_futureValue,
		final double p_k, final double p_time)
	{
		return p_futureValue / (1 + p_k * p_time);
	}

	public static double hypValDifference(final ResponseData p_response,
		final double p_k)
	{
		if(p_response.getResponseType() == ResponseType.NONE)
		{
			throw new RuntimeException(
				"Unable to calculate difference for null response");
		}

		final double value1 = hyperbolicPresentValue(
			p_response.getChoice1(), p_k,
			p_response.getChoice1Delay());
		final double value2 = hyperbolicPresentValue(
			p_response.getChoice2(), p_k,
			p_response.getChoice2Delay());
	...
	public static double lnProbOfResponse(final double p_a, final double p_k,
	...
	public static double lnProbOfResponses(final double p_a, final double p_k,
	...
	public static Double percentAttentionChecksCorrect(
		final ArrayList<ResponseData> p_responses)
	{
		...
		for(final ResponseData response: p_responses)
		{
			...
			else if(response.getChoice2() / response.getChoice1() > 30)
			{
				if(response.getResponseType() == ResponseType.LATER)
				{
					numCorrect++;
			...
			{
				System.out.println("Wrong: " + response.getChoice1() + ":"
					+ response.getChoice1Delay() + " vs "
					+ response.getChoice2() + ":" + response.getChoice2Delay()
					+ " choice " + response.getResponseType());
			}
		}
